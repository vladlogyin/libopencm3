...
void timer_enable_oc_clear(uint32_t timer_peripheral, enum tim_oc_id oc_id)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CCMR1(timer_peripheral) |= TIM_CCMR1_OC0CE;
		break;
	case TIM_OC1:
		TIM_CCMR1(timer_peripheral) |= TIM_CCMR1_OC1CE;
		break;
	case TIM_OC2:
		TIM_CCMR2(timer_peripheral) |= TIM_CCMR2_OC2CE;
		break;
	case TIM_OC3:
...
void timer_set_oc_fast_mode(uint32_t timer_peripheral, enum tim_oc_id oc_id)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CCMR1(timer_peripheral) |= TIM_CCMR1_OC0FE;
		break;
	case TIM_OC1:
		TIM_CCMR1(timer_peripheral) |= TIM_CCMR1_OC1FE;
		break;
	case TIM_OC2:
		TIM_CCMR2(timer_peripheral) |= TIM_CCMR2_OC2FE;
		break;
	case TIM_OC3:
		       ...
		       enum tim_oc_mode oc_mode)
{
	...
	switch (oc_id) {
	case TIM_OC0:
		shift = TIM_CCMR_OC0_SHIFT;
		cc_register = &TIM_CCMR1(timer_peripheral);
		break;
	case TIM_OC1:
		shift = TIM_CCMR_OC1_SHIFT;
		cc_register = &TIM_CCMR1(timer_peripheral);
		break;
	case TIM_OC2:
		shift = TIM_CCMR_OC2_SHIFT;
		cc_register = &TIM_CCMR2(timer_peripheral);
		break;
	case TIM_OC3:
		shift = TIM_CCMR_OC3_SHIFT;
		cc_register = &TIM_CCMR2(timer_peripheral);
		break;
	default:
		return;
	...
	*cc_register &= ~(
		(
			(TIM_CCMR_CCS_MASK<<TIM_CCMR_CCS_SHIFT)|
			(TIM_CCMR_OCM_MASK<<TIM_CCMR_OCM_SHIFT)
		)<<shift);
	*cc_register |=
		(
			(TIM_CCMR_CCS_OUT<<TIM_CCMR_CCS_SHIFT)|
			(oc_mode<<TIM_CCMR_OCM_SHIFT)
		)<<shift;
}
...
void timer_enable_oc_preload(uint32_t timer_peripheral, enum tim_oc_id oc_id)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CCMR1(timer_peripheral) |= TIM_CCMR1_OC0PE;
		break;
	case TIM_OC1:
...
tim_reg_base
@param[in] oc_id enum ::tim_oc_id OC channel designators
		TIM_OCx where x=0..3, TIM_OCxN where x=0..3
*/
...
void timer_set_oc_polarity_high(uint32_t timer_peripheral, enum tim_oc_id oc_id)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CCER(timer_peripheral) &= ~TIM_CCER_CC0P;
		break;
	case TIM_OC1:
	...
	case TIM_OC3:
		...
		break;
	case TIM_OC0N:
		TIM_CCER(timer_peripheral) &= ~TIM_CCER_CC0NP;
		break;
	case TIM_OC1N:
...
tim_reg_base
@param[in] oc_id enum ::tim_oc_id OC channel designators
		TIM_OCx where x=0..3, TIM_OCxN where x=0..2
*/
...
void timer_enable_oc_output(uint32_t timer_peripheral, enum tim_oc_id oc_id)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CCER(timer_peripheral) |= TIM_CCER_CC0E;
		break;
	case TIM_OC1:
	...
	case TIM_OC3:
		...
		break;
	case TIM_OC0N:
		TIM_CCER(timer_peripheral) |= TIM_CCER_CC0NE;
		break;
	case TIM_OC1N:
				 ...
				 enum tim_oc_id oc_id)
{
	switch (oc_id) {
	case TIM_OC0:
		TIM_CR2(timer_peripheral) |= TIM_CR2_OIS0;
		break;
	case TIM_OC0N:
		TIM_CR2(timer_peripheral) |= TIM_CR2_OIS0N;
		break;
	case TIM_OC1:
			 ...
			 enum tim_ic_filter flt)
{
	switch (ic) {
	case TIM_IC0:
		TIM_CCMR1(timer_peripheral) &= ~TIM_CCMR1_IC0F_MASK;
	...
	case TIM_IC1:
		...
		break;
	case TIM_IC2:
		TIM_CCMR2(timer_peripheral) &= ~TIM_CCMR2_IC2F_MASK;
			    ...
			    enum tim_ic_psc psc)
{
	switch (ic) {
	case TIM_IC0:
		TIM_CCMR1(timer_peripheral) &= ~TIM_CCMR1_IC0PSC_MASK;
	...
	case TIM_IC1:
		...
		break;
	case TIM_IC2:
		TIM_CCMR2(timer_peripheral) &= ~TIM_CCMR2_IC2PSC_MASK;
...
(TI2 for channel 0, TI1 for channel 1, TI4 for channel 2, TI3 for channel 3)
			...
			enum tim_ic_input in)
{
	...
	switch (ic) {
	case TIM_IC0:
		TIM_CCMR1(timer_peripheral) &= ~TIM_CCMR1_CC0S_MASK;
	...
	case TIM_IC1:
		...
		break;
	case TIM_IC2:
		TIM_CCMR2(timer_peripheral) &= ~TIM_CCMR2_CC2S_MASK;
